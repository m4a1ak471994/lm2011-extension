...
replication pipeline actually consumes; and writes the four input .dta files:
  ...
  input/crsp_daily_1993_2026.dta          ← CRSP daily + FF5 daily merged
  input/crsp_monthly_1993_2026.dta        ← CRSP monthly (subset of columns)
...
# Date window: cover [-252,-6] before earliest filings and [+6,+252] after latest.
# Extended to 2026 so the out-of-sample analysis can reach filings filed
# through 2026 Q1.
START_YR = 1992
END_YR = 2026
...
def build_crsp_monthly(conn: "wrds.Connection") -> None:
    ...
    print(f"  rows: {len(df):,}")
    _save_dta(df, OUT / "crsp_monthly_1993_2026.dta")
...
def build_crsp_daily(conn: "wrds.Connection") -> None:
    ...
    _save_dta(daily, OUT / "crsp_daily_1993_2026.dta")
...
def build_13f_instOwn(conn: "wrds.Connection") -> None:
    ...
    crspm = pd.read_stata(OUT / "crsp_monthly_1993_2026.dta",
