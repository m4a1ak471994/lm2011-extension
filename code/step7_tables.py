...
def write_diagnostic(t2: pd.DataFrame, t4: pd.DataFrame, t5: pd.DataFrame,
    ...
    md = []
    fmt = lambda x: f"{x:.4f}"
    md.append("# Methodology diagnostic — extended sample (1994–2024)\n")
    md.append(f"Analysis sample: **{n_sample:,} firm-years** / **{n_firms:,} unique permnos**.")
    md.append("LM (2011) in-sample reference: 50,115 firm-years / 8,341 firms.\n")
    ...
    md.append("```\n")
    md.append("## Table IV — Excess-return regressions, full 10-K (mine)\n")
    md.append("```")
    md.append(t4.to_string(index=False, float_format=fmt))
    md.append("```\n")
    md.append("## Table V — Excess-return regressions, MD&A only (mine)\n")
    md.append("```")
