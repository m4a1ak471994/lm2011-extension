...
CRSP_DAILY = INP / _os_path.environ.get("LM_CRSP_DAILY", "crsp_daily_1993_2026.dta")
CRSP_MONTHLY = INP / _os_path.environ.get("LM_CRSP_MONTHLY", "crsp_monthly_1993_2026.dta")
...
def compute_window_vars(sample: pd.DataFrame, crspd: pd.DataFrame) -> pd.DataFrame:
    ...
    For each filing in sample, compute (per LM 2011 Appendix Variable defs):
        excret_01      : BH compounded [0,+1] excess return (2-day),  in PERCENT
        excret_03      : BH compounded [0,+3] excess return (4-day) — LM Tables IV/V
        excret_05      : BH compounded [0,+5] excess return (6-day)
        excret_10      : BH compounded [0,+10] excess return (11-day)
        excret_20      : BH compounded [0,+20] excess return (21-day, ≈1 month)
    ...
    # Multiple event windows (in trading days from the filing date d=0).
    # Each is a BH compounded excess return: prod(1+ret) − prod(1+vwretd), in PERCENT.
    # excret_03 is the canonical LM (2011) [0,+3] 4-day window; the others are
    # robustness probes (does the negative-tone signal show up in the very-short
    # 2-day window? does it persist out to a 1-month window?).
    EVENT_LENGTHS = [2, 4, 6, 11, 21]   # 2-day = [0,+1] ; 21-day ≈ 1 calendar month
    EVENT_COLS = {n - 1: f"excret_{n-1:02d}" for n in EVENT_LENGTHS}
    excret_dict = {col: np.full(len(sample), np.nan) for col in EVENT_COLS.values()}
    size_b = np.full(len(sample), np.nan)
    share_turnover = np.full(len(sample), np.nan)
    ff_alpha = np.full(len(sample), np.nan)
    max_horizon = max(EVENT_LENGTHS)
    ...
    for i, r in enumerate(sample.itertuples(index=False)):
        ...
        # Event window: first trading day on/after d, take up to max_horizon days
        ev_mask = g["date"] >= d
        if not ev_mask.any():
            continue
        ev_all = g.loc[ev_mask].head(max_horizon)
        # Compute each event-window BH return that has full coverage
        for length in EVENT_LENGTHS:
            if len(ev_all) < length:
                continue
            ev = ev_all.head(length)
            if ev["ret"].isna().any() or ev["vwretd"].isna().any():
                continue
            bh_firm = float(np.prod(1.0 + ev["ret"].values) - 1.0)
            bh_mkt  = float(np.prod(1.0 + ev["vwretd"].values) - 1.0)
            excret_dict[EVENT_COLS[length - 1]][i] = (bh_firm - bh_mkt) * 100.0

        # The 4-day [0,+3] window — used by LM Tables IV/V — must exist for the
        # event-study sample. (The other windows can be NaN; downstream regressions
        # drop NaNs.)
        ev4 = ev_all.head(4)
        if len(ev4) < 4:
            continue
    ...
    out = sample.copy()
    for col, arr in excret_dict.items():
        out[col] = arr
    out["size_b"] = size_b
...
def main() -> None:
    ...
    # Year range: 1993 → 2026 covers all filings 1994-2026Q1 with their
    # [−252,−6] pre-window and [+6,+252] post-window event-study controls.
    crspd = crspd[crspd["date"].dt.year.between(1993, 2026)].copy()
    ...
    keep_cols = [
        ...
        # market controls + event-window CARs (excret_03 = LM canonical; others = robustness)
        "excret_01", "excret_03", "excret_05", "excret_10", "excret_20",
        "size_b", "log_size", "bm", "log_bm",
